...
def upload_file():
    ...
    time = request.form.get('time', 'N/A') # Default to 'N/A' if empty
    max_marks = request.form.get('max_marks', 'N/A') # Default to 'N/A' if empty
    ...
    if file and allowed_file(file.filename):
        ...
        return f"""
            <style> body {{ font-family: sans-serif; background-color: #1a1a1a; color: #e0e0e0; padding: 40px; }} h1 {{ color: #4CAF50; }} p {{ color: #bbb; }} a {{ display: inline-block; margin-top: 20px; padding: 10px 15px; background-color: #4CAF50; color: white; text-decoration: none; border-radius: 4px; }} a:hover {{ background-color: #45a_049; }} </style>
