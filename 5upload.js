document.addEventListener('DOMContentLoaded', function () {
    ...
    // --- NEW: Function to validate a single form card ---
    function validateForm(card) {
        ...
        for (const input of requiredInputs) {
            if (!input.value) {
                isValid = false;
                break; // Stop checking as soon as one is found empty
            }
        }
        ...
        if (!isValid) {
            ...
            // Reset error style if form is now valid
    ...
    // --- MODIFIED: "Upload All" button now validates first ---
    uploadAllBtn.addEventListener('click', async () => {
        for (const [identifier, data] of addedFiles.entries()) {
            // Only upload if not already uploaded AND if the form is valid
